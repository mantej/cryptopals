...
def bitflipping_attack(ciphertext, pos, injection):
    intermediate_byte = ord(ciphertext.decode('hex')[pos]) ^ ord("*")
    injected_byte = intermediate_byte ^ ord(injection)
    ciphertext = ciphertext[:(pos)*2] + chr(injected_byte).encode('hex') + ciphertext[(pos+1)*2:]
    return ciphertext
...
ciphertext = bitflipping_attack(ciphertext, 32, ";")
ciphertext = bitflipping_attack(ciphertext, 38, "=")
